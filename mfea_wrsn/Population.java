...
public class Population {
	...
	ListSensors list ;
	int numOfTask = 3;
	public Population(int populationSize, boolean initialize, ListSensors list) {
		this.list = list;
		individuals = new Individual[populationSize];
		if(initialize) {
			for(int i = 0; i < populationSize;i++) {
				Individual newIndi = new Individual(list.numberOfSensor());
				newIndi.initialazeIndividual(list);
	...
	public void assignCoordinates(Individual individual, double[] X, double []Y,double[] p,int flag) {
		for(int i = 0; i < X.length; i++) {
			individual.getSensor(i).x[flag] = X[individual.getSensor(i).getId() - 1];
			individual.getSensor(i).y[flag] = Y[individual.getSensor(i).getId() - 1];
			individual.getSensor(i).p[flag] = p[individual.getSensor(i).getId() - 1];
		}
	}
